...
//			mlSetStunServer(0,NULL);
			...
			printf("Trying stun: %s\n",stun_server);
			if(mlSetStunServer(stun_port, stun_server) == 0) 
				unsetStunServer(); // TODO se uno stun fallisce il lookup allora tutta la procedura fallisce
		}
		free(stun_server);
		break;
...
//	}
	...
	stun_server = STUN_SERVER_DEFAULT;//config_value_str_default(cfg_tags, "stun_server", STUN_SERVER_DEFAULT);
...
//	config_value_double(cfg_tags, "RTXholtdingtime", &RTXholtdingtime);
	...
	stun_servers[0] = strdup(STUN_SERVER_DEFAULT);
	for (i = 1 ; i<STUN_SERVERS_MAX ; i++) {
