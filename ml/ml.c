...
#define INADDR_NONE_STR "255.255.255.255"
...
int mlCompareAddr(struct sockaddr_storage* addr1,struct sockaddr_storage* addr2) {
	char buff1[INET6_ADDRSTRLEN];
	char buff2[INET6_ADDRSTRLEN];

	get_sockaddr_ip(addr1,buff1,ADDRESS_STR_LEN(addr1));
	get_sockaddr_ip(addr2,buff2,ADDRESS_STR_LEN(addr2));

	return strcmp(buff1,buff2) & strcmp(buff1,"") & strcmp(buff2,"");
}
...
{
	return !mlCompareAddr(&(sock1->external_addr),&(sock2->external_addr));
}
...
{
	static char s[INET6_ADDRSTRLEN+1+5+1+INET6_ADDRSTRLEN+1+5+1];
...
//	stun_server.sin_addr.s_addr = INADDR_NONE;
	init_sockaddr(&stun_server,6666,INADDR_NONE_STR);
}

bool isStunDefined()
{
//	return stun_server.sin_addr.s_addr != INADDR_NONE;
	char ip_a[ADDRESS_STR_LEN(&stun_server)];
	get_sockaddr_ip(&stun_server,ip_a,ADDRESS_STR_LEN(&stun_server));
//	printf("Evaluating if STUN is defined:\n%s <> %s\n",ip_a,INADDR_NONE_STR);
	return strcmp(ip_a,INADDR_NONE_STR) && strcmp(ip_a,""); 
}
...
void send_msg(int con_id, int msg_type, void* msg, int msg_len, bool truncable, send_params * sParams) {
	...
	int i=0;
#endif
	struct sockaddr_storage udpgen;
	bool retry;
	int pkt_len, offset;
	struct iovec iov[4];
	...
	do{
		...
		do {
			...
			switch(queueOrSendPacket(socketfd, iov, 4, &udpgen,priority)) {
				case MSGLEN:
...
int sendPacket(const int udpSocket, struct iovec *iov, int len, struct sockaddr_storage *socketaddr) {
	//monitoring layer hook
...
void recv_conn_msg(struct msg_header *msg_h, char *msgbuf, int msg_size, struct sockaddr_storage *recv_addr)
{
	...
	if (msg_size < sizeof(struct conn_msg)) {
		...
		get_sockaddr_ip(recv_addr,recv_addr_str,ADDRESS_STR_LEN(recv_addr));
		info("Invalid conn_msg received\n");
	...
	// if recv_addr is not in the header
	if (mlCompareAddr(&(con_msg->sock_id.internal_addr),recv_addr) &&
		mlCompareAddr(&(con_msg->sock_id.external_addr),recv_addr)) {
...
//				connectbuf[free_con_id]->external_socketID.external_addr.udpaddr.sin_family=AF_INET;
				...
				connectbuf[free_con_id]->internal_connect =
					!(mlCompareAddr(&(con_msg->sock_id.internal_addr),recv_addr));
				con_id = free_con_id;
			}
...
//	fprintf(stderr,"Stun Msg return value: %d\n",returnValue);
//	fprintf(stderr,"Stun Msg return reflex addr: %s\n",ipaddr);
	if (returnValue == 0) {
		/*
		...
		*/
		...
		inet_ntop(AF_INET, &(((struct sockaddr_in *) &reflexiveAddr)->sin_addr), ipaddr, INET_ADDRSTRLEN);
		init_sockaddr(&(local_socketID.external_addr),resp.mappedAddress.ipv4.port,ipaddr);
		NAT_traversal = true;
...
{
fprintf(stderr,"NAT traversal timeout\n");
debug("X. NatTrTo %d\n", NAT_traversal);
...
char* resolve(const char *hostname)
{
	...
	resolve_hostname(ipaddr,hostname,INET6_ADDRSTRLEN);
	return ipaddr;
}
...
{
	struct sockaddr_storage udpaddr = {0};
        debug("X. create_socket %s, %d\n", ipaddr, port);

	memset(&udpaddr,0,sizeof(struct sockaddr_storage));	//this will be sent over the net, so set it to 0
	init_sockaddr(&udpaddr,port,ipaddr);
	local_socketID.internal_addr = udpaddr;
...
void try_stun()
{
	if (isStunDefined()) {
		...
		*/
		...
		NAT_traversal = false;
//	memmove(&(local_socketID->external_addr),&(local_socketID->internal_addr),sizeof(local_socketID.internal_addr));
	} else {
		/*
...
//	printf("SIZE OF SOCKET_ID: %d",sizeof(socket_ID));
	recv_data_callback = recv_data_cb;
	mlSetRecvTimeout(timeout_value);
	if (stun_ipaddr) {
		 if(mlSetStunServer(stun_port, stun_ipaddr) == 0)
			{
				unsetStunServer();
			}
	} else {
...
void printAddr() {
	char internal_addr[INET6_ADDRSTRLEN];
	char external_addr[INET6_ADDRSTRLEN];

	get_sockaddr_ip(&(local_socketID.internal_addr),internal_addr,ADDRESS_STR_LEN(&(local_socketID.internal_addr)));
	get_sockaddr_ip(&(local_socketID.external_addr),external_addr,ADDRESS_STR_LEN(&(local_socketID.external_addr)));

	fprintf(stderr,"Internal Address: %s\n",internal_addr);
	fprintf(stderr,"External Address: %s\n",external_addr);
}
...
int mlSocketIDToString(socketID_handle socketID,char* socketID_string, size_t len){
	...
	char port_delim = '_';
	assert(socketID); // TODO Why not return 1?

	get_sockaddr_ip(&(socketID->internal_addr),internal_addr,ADDRESS_STR_LEN(&(socketID->internal_addr)));
	get_sockaddr_ip(&(socketID->external_addr),external_addr,ADDRESS_STR_LEN(&(socketID->external_addr)));

	internal_port =	get_sockaddr_port(&(socketID->internal_addr));
	external_port = get_sockaddr_port(&(socketID->external_addr));
//	fprintf(stderr,"Handle - Internal Address: %s\n",internal_addr);
//	fprintf(stderr,"Handle - External Address: %s\n",external_addr);
//
//	Following a quick and dirty trick...
	if (strcmp(external_addr,"") == 0)
		strcpy(external_addr,internal_addr);
	if (external_port = -1)
		external_port = internal_port;

	port_delim = (address_family(internal_addr)==AF_INET) ? ':' : '_';
	

	snprintf(socketID_string,len,"%s%c%d-%s%c%d", 
		internal_addr,port_delim, internal_port,
		external_addr,port_delim, external_port);

//	fprintf(stderr,"handle: %p\n",socketID);
//	fprintf(stderr,"local pointer: %p\n",&local_socketID);
...
/* prototype 192.168.0.1:6666-193.213.40.12:6666
	 prototype fe80::23:fe01_6666-fe80::23:fe01_6666 */
	...
	port_delim = (strchr(socketID_string,port_delim)==NULL) ? ':' : '_';	

	char *pch;
	char *s = strdup(socketID_string);

	//replace '_' with a blank
	pch=strchr(s,port_delim);
	while (pch!=NULL){
				*pch = ' ';
		pch=strchr(pch+1,port_delim);
	}
	pch=strchr(s,'-');
	if(pch) *pch = ' ';
...
//	fprintf(stderr,"int_addr: %s, int_port: %d, ext_addr: %s, ext_port: %d\n",internal_addr,internal_port,external_addr,external_port);
	...
	init_sockaddr(&(socketID->internal_addr),internal_port,internal_addr);
	init_sockaddr(&(socketID->external_addr),external_port,external_addr);
...
int mlHashSocketID(socketID_handle sock) {
	//assert(sock);
	return get_sockaddr_port(&(sock->internal_addr)) +
		get_sockaddr_port(&(sock->external_addr));
}

int mlCompareSocketIDs(socketID_handle sock1, socketID_handle sock2) {
	int res = 0;

	assert(sock1 && sock2); // TODO Why?
	char str1[500], str2[500];
	mlSocketIDToString(sock1,str1,500);
	mlSocketIDToString(sock2,str2,500);
	/*
	* compare internal addr
	if(sock1 == NULL || sock2 == NULL)
		return 1;

	res = mlCompareSocketIDsByPort(sock1,sock2);

	if (res == 0)
		res = mlCompareAddr(&(sock1->internal_addr),&(sock2->internal_addr));

	if (res == 0)
	res = mlCompareAddr(&(sock1->external_addr),&(sock2->external_addr));
	*/
...
int mlCompareSocketIDsByPort(socketID_handle sock1, socketID_handle sock2)
{
	...
	port1 = get_sockaddr_port(&(sock1->internal_addr));
	port2 = get_sockaddr_port(&(sock2->internal_addr));
	if( port1 > 0 && port2 > 0 && port1 != port2)
			return 1;

	port1 = get_sockaddr_port(&(sock1->external_addr));
	port2 = get_sockaddr_port(&(sock2->external_addr));
	if( port1 > 0 && port2 > 0 && port1 != port2)
			return 1;
...
/* setter  */
int mlSetStunServer(const int port,const char *stun_host){
	char* ipaddr = resolve(stun_host);
	int res;
	res =	init_sockaddr((struct sockaddr_storage *)&stun_server,port,ipaddr);
	free(ipaddr);
	return res;
}
...
int mlGetExternalIP(char* external_addr){
	...
	struct sockaddr_storage udpgen = local_socketID.external_addr;

	get_sockaddr_ip((struct sockaddr_storage *)&udpgen,external_addr,ADDRESS_STR_LEN(&udpgen));
	
	return (external_addr == NULL) ? -1 : 0;
